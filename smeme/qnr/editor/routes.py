"""QNR Editor Routes - Write operations for graph editing."""
...
from smeme.qnr.editor import tools_panel as tools_panel_handlers
from smeme.qnr.editor.models import (
    CreateEdgeRequest,
    CreateNodeRequest,
...
router = APIRouter(prefix="/qnr/editor", tags=["qnr_editor"])
...
def render_edit_blocked_error(qnr_id: UUID, error_detail: str) -> HTMLResponse:
    """
    ...
    error_html = render_callout_html(
        ...
        body=(
            ...
            f'<a href="/qnr/editor/{qnr_id}/create_version" '
...
def _side_panel_validation_html(
    *,
    ...
    if not (
        ...
        return ""
    panel = templates.env.get_template("qnr/_validation_panel.html").render(
        {
...
def render_editor_oob_swaps(
    ...
    graph_checklist_html = templates.env.get_template("qnr/_graph_checklist.html").render(
        {
            "graph": viewer_result["graph"],
            "qnr_id": qnr_id,
            "selected_node_id": selected_node_id,
    ...
    is_public = viewer_result.get("is_public", False)
    validation_badge_html = templates.env.get_template("qnr/_validation_badge.html").render(
        {
            "validation_data": validation_data,
            "is_public": is_public,
        }
    ...
    tools_chip_html = templates.env.get_template("qnr/_editor_tools_chip.html").render(
        {"tools_row_state": viewer_result.get("tools_row_state", "not_built")}
    )

    editor_view = viewer_result.get("editor_view", "graph")
    status_bar_html = templates.env.get_template("qnr/_editor_status_bar.html").render(
        {
    ...
    warning_banner_html = ""
    if warnings:
        warning_banner_html = templates.env.get_template("qnr/_warning_banner.html").render(
            {"warnings": warnings}
        )
...
async def _create_node_error_panel_html(
    *,
    ...
    oob_swaps = render_editor_oob_swaps(viewer_result, qnr_id, selected)
...
async def enforce_versioning_for_public_edits(
    ...
    If the QNR is currently public OR was ever public, BLOCKS editing entirely.
    Author must explicitly create a new version using the "Create New Version" button.

    If the QNR is private and was never public, returns the original QNR for direct editing.

    Args:
        db: Database session
        qnr: The QNR being edited
        user_id: The user attempting to edit

    Returns:
        QNR to edit (original if private and was never public)

    Raises:
        HTTPException: If QNR is archived or public/was_ever_public (cannot edit directly)
    """
    ...
    # If QNR is currently public OR was ever public, BLOCK editing
    if qnr.is_public or qnr.was_ever_public:
        reason = "public" if qnr.is_public else "previously public"
        raise HTTPException(
            status_code=403,
    ...
    # QNR is private and was never public - allow direct editing
    return qnr
...
async def authorize_workflow_edit(
    ...
    return await enforce_versioning_for_public_edits(db, qnr, user.id)
...
async def create_node(
    ...
    try:
        qnr = await authorize_workflow_edit(db, qnr, user)
    except HTTPException as e:
        if e.status_code == 403:
            return render_edit_blocked_error(qnr_id, e.detail)
        raise
    ...
    operation_data = req.model_dump(exclude={"qnr_id", "panel_context_node_id"})
    ...
    oob_swaps = render_editor_oob_swaps(viewer_result, qnr_id, resolved_node_id)
...
async def update_node(
    ...
    """
    qnr_id = req.qnr_id
    logger.info(f"Updating node: {req.node_id}", extra={"qnr_id": str(qnr_id)})
    ...
    # Exclude qnr_id as it's passed separately to the workflow
    operation_data = req.model_dump(exclude_none=True, exclude={"qnr_id"})
    ...
    # Combine side panel content + OOB swaps for all editor UI elements
    oob_swaps = render_editor_oob_swaps(viewer_result, qnr_id, req.node_id)
    combined_html = f"{viewer_result['rendered_html']}{oob_swaps}"
...
async def delete_node(
    ...
    # Exclude qnr_id as it's passed separately to the workflow
    operation_data = req.model_dump(exclude={"qnr_id"})

    logger.info(
        f"Deleting node: {req.node_id}",
        extra={"qnr_id": str(qnr_id), "operation_data": operation_data},
    )
    ...
    # Combine side panel content + OOB swaps for all editor UI elements
    oob_swaps = render_editor_oob_swaps(viewer_result, qnr_id, None)
    combined_html = f"{viewer_result['rendered_html']}{oob_swaps}"

    return HTMLResponse(content=combined_html)


@router.get("/validate/{qnr_id}", response_class=HTMLResponse)
...
async def create_edge(
    ...
    """
    qnr_id = req.qnr_id
    logger.info(f"Creating edge: {req.source} -> {req.target}", extra={"qnr_id": str(qnr_id)})
    ...
    if not result.get("success", False):
        ...
        logger.error(
            ...
            extra={"qnr_id": str(qnr_id), "source": req.source, "target": req.target},
        )
    ...
    # Combine side panel content + OOB swaps for all editor UI elements
    oob_swaps = render_editor_oob_swaps(viewer_result, qnr_id, req.source)
    combined_html = f"{viewer_result['rendered_html']}{oob_swaps}"
...
async def update_edge(
    ...
    """
    qnr_id = req.qnr_id
    logger.info(f"Updating edge: {req.source} -> {req.old_target}", extra={"qnr_id": str(qnr_id)})
    ...
    if not result.get("success", False):
        ...
        logger.error(
            ...
            extra={"qnr_id": str(qnr_id), "source": req.source, "old_target": req.old_target},
        )
...
async def delete_edge(
    ...
    """
    qnr_id = req.qnr_id
    logger.info(f"Deleting edge: {req.source} -> {req.target}", extra={"qnr_id": str(qnr_id)})
...
@router.post("/{qnr_id}/settings", response_class=HTMLResponse)
async def update_qnr_settings(
    qnr_id: UUID,
    user: CurrentUser,
    db: AsyncSessionDep,
    ...
    """Update QNR metadata settings (intended_audience, use_case)."""
    qnr = await get_qnr_by_id(db, qnr_id)
    if not qnr:
        raise HTTPException(status_code=404, detail="Workflow not found")
    if qnr.author_id != user.id:
    ...
    qnr.intended_audience = (
        intended_audience.strip() if intended_audience and intended_audience.strip() else None
    )
    qnr.use_case = use_case.strip() if use_case and use_case.strip() else None

    db.add(qnr)
    await db.commit()

    logger.info(
        f"Updated QNR settings: {qnr_id}",
        extra={
            "qnr_id": str(qnr_id),
...
@router.post("/{qnr_id}/research-corpus", response_class=HTMLResponse)
async def save_research_corpus(
    ...
    row = await db.get(QnrResearchCorpus, qnr_id)
    if row:
        row.body_text = normalized
    else:
        db.add(QnrResearchCorpus(qnr_id=qnr.id, body_text=normalized))
    db.add(qnr)
    await db.commit()
...
@router.delete("/{qnr_id}/research-corpus", response_class=HTMLResponse)
async def delete_research_corpus(
    ...
    await db.execute(delete(QnrResearchCorpus).where(QnrResearchCorpus.qnr_id == qnr_id))
    await db.commit()
    return HTMLResponse(
...
async def edit_title_form(
    ...
    """
    Return inline edit form for QNR title.
    """
    # Authorization check
    qnr = await get_qnr_by_id(db, qnr_id)
    if not qnr:
    ...
    <div class="flex items-center gap-2">
      <form hx-post="/qnr/editor/update_title"
...
async def update_title(
    ...
    """
    Update QNR title and return updated title display.
    """
    # Authorization check
    qnr = await get_qnr_by_id(db, qnr_id)
    if not qnr:
    ...
    logger.info(
        f"Updated QNR title: {qnr_id} -> '{title}'",
    ...
    <div class="flex items-center gap-2">
      ...
      <button class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors"
              hx-post="/qnr/editor/edit_title_form"
...
@router.get("/{qnr_id}/publish-modal", response_class=HTMLResponse)
async def publish_confirm_modal(
    ...
    return templates.TemplateResponse(
        "qnr/_publish_confirm_modal.html",
        {
            "request": request,
            "qnr_id": str(qnr_id),
...
def _publish_gate_failed_response(
    request: Request, readiness: PublishReadiness, qnr_id: UUID
) -> HTMLResponse:
    """HTMX: swap modal + OOB editor banner; non-HTMX: full page with back link."""
    is_htmx = (request.headers.get("HX-Request") or "").lower() == "true"
    ctx = {"request": request, "readiness": readiness, "qnr_id": str(qnr_id)}
    if is_htmx:
        return templates.TemplateResponse(
            "qnr/_publish_blocked_modal.html",
            ctx,
            status_code=200,
        )
    return templates.TemplateResponse(
...
@router.get("/{qnr_id}/publish-preflight", response_class=HTMLResponse)
async def publish_preflight_panel(
    ...
    return templates.TemplateResponse(
        "qnr/_publish_preflight_panel.html",
        {
            "request": request,
            "readiness": readiness,
            "qnr_id": str(qnr_id),
...
def _publish_success_redirect_url(qnr_id: UUID, return_next: str | None) -> str:
    """Redirect after successful deploy; honors dashboard / tools tab return_next."""
    if return_next == "dashboard":
        return "/qnr/dashboard?deployed=1"
    if return_next in ("tools", "editor"):
        return f"/qnr/{qnr_id}/editor?view=tools&reasoning_compiled=1"
    return f"/qnr/{qnr_id}/editor?reasoning_compiled=1"


@router.post("/{qnr_id}/publish", response_class=HTMLResponse, response_model=None)
async def publish_qnr(
    ...
    """
    Compile this QNR to a persisted IR artifact (sets reasoning_status = "compiled").

    Does NOT change gallery visibility. MCP deploy is available on all tiers.
    """
    logger.info(f"Publishing reasoning artifact for QNR: {qnr_id}", extra={"qnr_id": str(qnr_id)})
    ...
    result = await db.execute(
        select(QNR)
        .options(selectinload(QNR.parent), selectinload(QNR.children))
        .where(QNR.id == qnr_id)
    )
    ...
    if not readiness.ready:
        return _publish_gate_failed_response(request, readiness, qnr_id)
    ...
    corp_row = await get_qnr_research_corpus_row(db, qnr.id)
    ...
    cev_contract, corpus_snapshot = induce_published_evidence_contract_at_publish(
        ...
        legal_at_publish=bool(qnr.cevi_legal),
    )
    ...
    logger.info(
        f"Reasoning artifact compiled for QNR: {qnr.title}",
        extra={
            "qnr_id": str(qnr.id),
    ...
    redirect_url = _publish_success_redirect_url(qnr_id, return_next)
...
@router.get("/{qnr_id}/tools", response_class=HTMLResponse, response_model=None)
async def editor_tools_panel(
    ...
    return await tools_panel_handlers.serve_tools_panel(
        request=request, qnr_id=qnr_id, user=user, db=db
    )


@router.get("/{qnr_id}/tools-checks", response_class=HTMLResponse, response_model=None)
...
async def update_edge_form(
    ...
    form_html = f"""
    <form hx-post="/qnr/editor/update_edge"
          hx-target="#side-panel-content"
          hx-swap="innerHTML"
          hx-ext="response-targets"
...
async def create_new_version(
    ...
    Used when QNR is public or was ever public and cannot be edited directly.
    ...
    logger.info(
        f"Creating new version of QNR: {original.title}",
        extra={
    ...
    root_qnr_id = await _get_root_qnr_id(db, original)
    root_qnr = await get_qnr_by_id(db, root_qnr_id)
    if not root_qnr:
        raise HTTPException(status_code=404, detail="Root workflow not found")
    # Strip any existing version suffix (e.g., "My QNR v1" → "My QNR")
