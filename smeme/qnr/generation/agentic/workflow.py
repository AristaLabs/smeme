...
from smeme.qnr.generation.agentic.subgraphs.build import (
    create_build_subgraph,
    extract_build_input,
    merge_build_output,
)
from smeme.qnr.generation.agentic.subgraphs.conclusions import (
    create_conclusions_subgraph,
    extract_conclusions_input,
    merge_conclusions_output,
)
from smeme.qnr.generation.agentic.subgraphs.design import (
...
def set_design_phase(state: AgenticQNRGenerationState) -> dict:
...
def set_build_phase(state: AgenticQNRGenerationState) -> dict:
...
def set_complete_phase(state: AgenticQNRGenerationState) -> dict:
...
def set_error_phase(state: AgenticQNRGenerationState) -> dict:
...
# =============================================================================
# QNR Persistence Node (Sprint 6: Moved from nodes/build.py)
# =============================================================================
...
async def save_qnr_node(
    ...
    from smeme.core.models import QNR, QnrResearchCorpus
    ...
    try:
        ...
        if brief_title and brief_title.strip():
            qnr_title = brief_title.strip()[:200]
        else:
            qnr_title = graph_dict.get("metadata", {}).get("title", f"QNR: {user_prompt[:50]}...")

        # Create QNR record (QNR has title; description lives in graph_data.metadata)
        qnr = QNR(
            title=qnr_title,
            author_id=user_id,
        ...
        if normalized_corpus:
            db.add(QnrResearchCorpus(qnr_id=qnr.id, body_text=normalized_corpus))

        await db.commit()
        await db.refresh(qnr)

        logger.info(
            "QNR saved successfully",
            extra={
                "user_id": str(user_id),
                "qnr_id": str(qnr.id),
...
async def wait_for_conclusions_edit_node(state: AgenticQNRGenerationState) -> dict:
    """
...
async def wait_for_design_edit_node(state: AgenticQNRGenerationState) -> dict:
    """
...
def should_continue_after_error(state: AgenticQNRGenerationState) -> str:
...
def route_after_build_subgraph(state: AgenticQNRGenerationState) -> str:
...
def route_after_research_edit(state: AgenticQNRGenerationState) -> str:
...
def build_agentic_generation_workflow() -> StateGraph:
    ...
    # Set Complete Phase → Save QNR → END
