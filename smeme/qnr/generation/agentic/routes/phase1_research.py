...
from smeme.qnr.generation.agentic.background import (
...
from smeme.qnr.generation.agentic.file_limits import MAX_FILES_PER_GENERATION, MAX_TOTAL_BYTES
...
from smeme.qnr.generation.agentic.streaming import get_bus, sse_event_stream
from smeme.qnr.generation.agentic.subgraphs.models import InterruptPayload
from smeme.qnr.generation.agentic.subgraphs.research import EXCLUDE_DOMAINS
...
def _brief_page_context(
    ...
    *,
    ...
    return {
        "request": request,
        "user": user,
        "main_content_template": "qnr/generation/_main_initial_form.html",
        ...
        "form_country": (
            form.get("country", "") if settings.show_qnr_generation_region_selector else ""
        ),
...
async def wizard_start_entry(
    ...
    if block is None:
        return RedirectResponse(url="/qnr/agentic/brief", status_code=303)

    if request.headers.get("HX-Request") == "true":
        return templates.TemplateResponse(
            "qnr/generation/_wizard_start_blocked_modal.html",
            {
                "request": request,
                "wizard_start_block": block,
                "stripe_configured": settings.stripe_configured,
    ...
    return templates.TemplateResponse(
        "qnr/generation/_generation_layout.html",
        _brief_page_context(
            request,
            user,
            block=block,
...
async def agentic_generation_brief_partial(
    ...
    block, in_progress_count = await _wizard_start_context(db, user)
    return templates.TemplateResponse(
        "qnr/generation/_main_initial_form.html",
        _brief_page_context(
            request,
            user,
            block=block,
...
async def get_research_edit_form(
    ...
    return render_step_template(
        request=request,
        main_content_template="qnr/generation/_main_research_edit.html",
        context=ctx,
        user=user,
    )
...
async def retry_research_generation(
    ...
    return render_step_template(
        request=request,
        main_content_template="qnr/generation/_main_research_loading.html",
        context={
            "thread_id": thread_id,
            "workflow_title": state.get("title", ""),
            "user_prompt": goal,
            "stream_url": f"/qnr/agentic/generate/{thread_id}/stream",
...
async def start_generation(
    ...
    if not settings.show_qnr_generation_region_selector:
        form_dict["country"] = ""
    ...
    if block:
        ...
        if is_htmx:
            return templates.TemplateResponse("qnr/generation/_main_initial_form.html", ctx)
        return templates.TemplateResponse("qnr/generation/_generation_layout.html", ctx)
...
async def submit_research_context(
    ...
    if action == "skip_conclusions":
        ...
        return render_step_template(
            request=request,
            main_content_template="qnr/generation/_main_conclusions_input.html",
            context={
                "thread_id": thread_id,
    ...
    try:
        ...
        if interrupts:
            ...
            return render_wizard_step_safe(
                request=request,
                main_content_template="qnr/generation/_main_conclusions_edit.html",
                context={
                    "thread_id": thread_id,
