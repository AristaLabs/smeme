...
async def _purge_checkpoints_for_user(db: AsyncSession, user_id: UUID) -> None:
    rows = (
        (
            await db.execute(
                select(InProgressDecisionTreeGeneration).where(InProgressDecisionTreeGeneration.user_id == user_id)
            )
        )
        .scalars()
        .all()
    ...
    if rows:
        await db.execute(
            delete(InProgressDecisionTreeGeneration).where(InProgressDecisionTreeGeneration.user_id == user_id)
        )


async def _purge_workflows(db: AsyncSession, user_id: UUID) -> None:
    roots = (
        (await db.execute(select(DecisionTree).where(DecisionTree.author_id == user_id, DecisionTree.parent_decision_tree_id.is_(None))))
        .scalars()
        .all()
    )
    for root in roots:
...
async def _purge_orphan_sessions(db: AsyncSession, user_id: UUID) -> None:
    session_ids = (
        (await db.execute(select(DecisionTreeSession.id).where(DecisionTreeSession.user_id == user_id)))
        .scalars()
        .all()
    )
    if not session_ids:
