...
**Shipped (CEVI Phase A — corpus + provenance):** durable **`decision_tree_research_corpora`** (per-DecisionTree research text), **`qnrs.cevi_legal`**, editor **GET/POST/DELETE** corpus routes + settings + publish preflight copy, **`research_corpus_hash`** on **`reasoning_compiled_artifacts`** (and matching **`provenance.research_corpus_hash`** in the frozen contract), agentic **save** merges pasted + factor research into the corpus row, and **`smeme/reasoning/cevi/`** (`atom_catalog`, `corpus_normalize`, `generation_corpus`, `induce_published_evidence_contract_at_publish`).
