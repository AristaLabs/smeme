...
* **DecisionTree / editor:** [`validate_graph_for_editing` / `validate_graph_for_publication`](../../decision-trees/helpers/validation.py) — authoring rules, conclusions, defaults, reasoning authoring contract (tier-2/3).
* **Publish gate (app):** [`assess_publish_readiness`](publish_readiness.py) — publication validation → `compile_dt_graph_to_ir` → `validate_ir` → `enumerate_conclusion_sat_queries`. Editor: `GET …/publish-preflight`, `POST …/publish`.
