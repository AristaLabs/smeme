...
async def publish_decision_tree(
    ...
    from smeme.reasoning.artifact_deploy import PublishGraphChangedError

    raise_if_workflow_edit_denied(user, decision_tree)

    try:
        artifact = await compile_and_persist_current_graph(db, decision_tree)
    except DeployNotReadyError as exc:
        return _publish_gate_failed_response(request, exc.readiness, decision_tree_id)
