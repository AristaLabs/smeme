"""Editor-specific models for DecisionTree graph modification.
...
class DecisionTreeEditorState(TypedDict):
    """
...
class CreateNodeRequest(BaseModel):
    ...
    **Single-entry graph:** the first question on an empty graph is the entry.
    ...
    ``POST /decision-trees/editor/create_node_wired`` for non-empty graphs instead.
    ...
    **Node id:** leave ``node_id`` blank for an opaque server id (``q_<hex>``);
    ...
    decision_tree_id: UUID = Field(description="DecisionTree ID")
    node_id: str = Field(
        default="",
