...
def bare_create_node_blocked_message(graph: DTGraph) -> str | None:
    ...
    return (
        ...
        "Use Add node in the side panel (POST /decision-trees/editor/create_node_wired), or add an edge from an existing question first."
    )


def validate_graph(graph: DTGraph) -> tuple[bool, str | None]:
    """
    Validate DecisionTree graph structure (strict validation).
    ...
    Conclusion Node Rules (per decision tree_CONCLUSION_NODES_PLAN.md):
...
def get_reachable_questions(graph: DTGraph, responses: dict[str, str]) -> set[str]:
    ...
    Args:
        graph: The DecisionTree graph structure
...
def validate_graph_for_generation(
    ...
    *,
    ...
    from smeme.decision_tree.generation.agentic.branching_quality import assess_branching_quality
