"""State models for agentic DecisionTree generation workflow.
...
DecisionTreeGenerationPhase = Literal[
...
class AgenticDecisionTreeGenerationState(TypedDict, total=False):
    """
    LangGraph workflow state for agentic DecisionTree generation.
    ...
    title: NotRequired[str]  # User-provided DecisionTree name (required from brief; used at save)
    ...
    # === Output ===
    decision_tree_id: NotRequired[str]  # UUID as string (saved DecisionTree)
