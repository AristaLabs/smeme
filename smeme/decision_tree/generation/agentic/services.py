"""Service layer for managing DecisionTree generation workflow sessions."""
...
class DecisionTreeGenerationCheckpointManager:
    """Manages lifecycle of in-progress DecisionTree generation sessions."""
    ...
    async def start_new_generation(
        ...
        generation = InProgressDecisionTreeGeneration(
            user_id=user.id,
        ...
        logger.info(
            "Started new DecisionTree generation",
            extra={
                "user_id": str(user.id),
                "thread_id": thread_id,
                "graph_version": graph_version,
    ...
    async def get_generation(
        ...
        stmt = select(InProgressDecisionTreeGeneration).where(
            InProgressDecisionTreeGeneration.id == generation_id,
    ...
    async def list_user_generations(
    ...
    ) -> list[InProgressDecisionTreeGeneration]:
        ...
        """
        stmt = (
            select(InProgressDecisionTreeGeneration)
            .where(InProgressDecisionTreeGeneration.user_id == user_id)
            .order_by(InProgressDecisionTreeGeneration.started_at.desc())
        )

        if not include_expired:
            stmt = stmt.where(InProgressDecisionTreeGeneration.expires_at > datetime.now(UTC))
    ...
    async def cleanup_expired_generations(
        ...
        stale_stmt = select(InProgressDecisionTreeGeneration).where(
            (InProgressDecisionTreeGeneration.expires_at < datetime.now(UTC))
            | (InProgressDecisionTreeGeneration.last_checkpoint_at < cutoff_time)
        )
...
# Global instance
checkpoint_manager = DecisionTreeGenerationCheckpointManager()
