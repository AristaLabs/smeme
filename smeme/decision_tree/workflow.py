"""LangGraph workflow for DecisionTree questionnaire navigation.
...
from smeme.decision_tree.workflow_state import DecisionTreeSessionState, SessionStateUpdate
...
async def load_decision_tree_node(state: DecisionTreeSessionState, config: RunnableConfig) -> SessionStateUpdate:
    ...
    decision_tree_id = UUID(state["decision_tree_id"])
    ...
    logger.info(
        ...
        extra={
            ...
            "user_id": user_id,
            "node": "load_decision_tree",
        },
    )

    # Try cache first
    graph = await get_cached_graph(decision_tree_id)
    if graph:
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(
            "DecisionTree loaded from cache",
            extra={
                "decision_tree_id": str(decision_tree_id),
                "session_id": session_id,
                "cache_hit": True,
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(
            "DecisionTree loaded from database and cached",
            extra={
                "decision_tree_id": str(decision_tree_id),
                "session_id": session_id,
                "cache_hit": False,
...
async def determine_next_question_node(
    ...
    # --- Handle "review" intent (reopen completed DecisionTree) ---
    if direction == "review":
        if responses:
...
async def render_question_node(
    ...
    template = jinja_env.get_template("decision_tree/_question.html")
...
async def _render_conclusion_node(
    ...
    template = jinja_env.get_template("decision_tree/_conclusion.html")
    html = template.render(
        conclusion=cdata,
        conclusion_id=conclusion_id,
        session_id=str(session.id),
    ...
    <div class="max-w-2xl mx-auto p-6">
        ...
        render_callout_html(
            ...
            body=(
                ...
                f'<div class="mt-4"><a href="/decision-trees/dashboard" class="text-brand-600 hover:underline text-sm">'
...
def route_after_determination(state: DecisionTreeSessionState) -> str:
...
def build_decision_tree_session_workflow() -> StateGraph:
    """
    ...
    """
    # Create workflow
    workflow = StateGraph(DecisionTreeSessionState)
    ...
    workflow.add_edge("load_decision_tree", "determine_next_question")
