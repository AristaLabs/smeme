...
- DTGraphMetadata: Graph-level metadata (title, description, etc.)
...
(smeme.decision_tree.editor.operations) to create transformed copies instead.
...
"""
Node types in a DecisionTree graph:
...
class ConclusionData(BaseModel):
    ...
    Conclusions represent the endpoint of a decision path in the DecisionTree.
    ...
    Design Principles (per decision tree_CONCLUSION_NODES_PLAN.md):
    ...
    - At least TWO conclusions required per decision tree
    """
...
class DTGraphMetadata(BaseModel):
    ...
    title: str = Field(description="DecisionTree title (required)")
    description: str | None = Field(default=None, description="DecisionTree description")
    category: str | None = Field(default=None, description="DecisionTree category")
...
class DTGraph(BaseModel):
    ...
    metadata: DTGraphMetadata = Field(description="Graph-level metadata (required)")
...
# ============================================================================
# In-Progress DecisionTree Generation Tracking (for persistent workflow state)
# ============================================================================


class InProgressDecisionTreeGeneration(BaseSQLModel, table=True):
    """Tracks in-progress DecisionTree generation workflows for user lookup.
    ...
    Lifecycle:
    - Created when user starts a new DecisionTree generation
    - Updated on each checkpoint (last_checkpoint_at)
    - Deleted when workflow completes or expires
    """

    __tablename__ = "in_progress_decision_tree_generations"
...
class WizardGenerationEvent(BaseSQLModel, table=True):
    ...
    generation_id: UUID | None = SQLField(
        default=None,
        nullable=True,
        description="InProgressDecisionTreeGeneration.id when known",
    )
    ...
    event_metadata: dict[str, Any] = SQLField(
        ...
        description="action, source, decision_tree_id, abandon reason, etc.",
    )
