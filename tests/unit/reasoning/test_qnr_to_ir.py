...
def test_compile_qnr_to_ir_structure_and_determinism():
    g = _radio_two_conclusions()
    a = compile_qnr_to_ir(g)
    b = compile_qnr_to_ir(g)
    assert a == b
...
def test_compile_default_edge_empty_guard_expr():
    g = QNRGraph(
        ...
        metadata=QNRMetadata(title="default edge"),
    )
