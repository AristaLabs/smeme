...
def _golden_radio_graph() -> QNRGraph:
    return QNRGraph(
        ...
        metadata=QNRMetadata(title="IR unit test"),
    )


_GOLDEN_QNR_ID = UUID("00000000-0000-4000-8000-000000000001")
_GOLDEN_DIGEST = (
...
def test_nfc_question_label_normalization_stable_digest() -> None:
    ...
    d1 = manifest_core_digest(build_manifest_core(g1, _GOLDEN_QNR_ID))
    d2 = manifest_core_digest(build_manifest_core(g2, _GOLDEN_QNR_ID))
...
def test_markdown_blind_audit_no_topology_leaks() -> None:
    g = _golden_radio_graph()
    mc = build_manifest_core(g, _GOLDEN_QNR_ID)
    md = render_manifest_markdown(
        manifest_core=mc,
        title="T",
        qnr_id=_GOLDEN_QNR_ID,
        slug="t",
    )
    assert "q1" in md
    assert "Yes" in md
...
def test_safe_worksheet_slug() -> None:
    ...
    assert safe_worksheet_slug("   ") == "qnr"
