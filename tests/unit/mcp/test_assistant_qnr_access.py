...
async def test_select_list_excludes_compiled_when_not_discoverable(test_session_factory):
    ...
    async with test_session_factory() as session:
        ...
        r = await session.execute(select_qnrs_for_assistant_tools_list(user.id))
        ...
        r2 = await session.execute(select_qnrs_for_assistant_tools_list(user.id))
        rows2 = list(r2.scalars().all())
        assert len(rows2) == 1
        assert rows2[0].id == q.id

        await session.execute(delete(QNR).where(QNR.id == q.id))
