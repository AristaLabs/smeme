...
async def _delete_workflow(client, decision_tree_id, *, confirm_phrase: str):
    return await client.delete(
        f"/decision-trees/{decision_tree_id}/delete",
...
async def delete_user(test_session_factory):
    ...
    async with test_session_factory() as session:
        ...
        current = DecisionTree(
            ...
            is_current=True,
            version_number=2,
            parent_decision_tree_id=root.id,
            is_archived=False,
        )
        session.add(current)
        await session.commit()
        ...
        decision_tree_session = DecisionTreeSession(user_id=user.id, decision_tree_id=current.id)
    ...
    async with test_session_factory() as session:
        ...
        await session.execute(delete(DecisionTreeSession).where(DecisionTreeSession.decision_tree_id.in_(family_ids)))
        await session.execute(
            delete(ReasoningCompiledArtifact).where(ReasoningCompiledArtifact.decision_tree_id.in_(family_ids))
        )
        await session.execute(
            update(DecisionTree).where(DecisionTree.id.in_(family_ids)).values(parent_decision_tree_id=None)
        )
        await session.execute(delete(DecisionTree).where(DecisionTree.id.in_(family_ids)))
...
async def test_delete_wrong_phrase_returns_400(client, app_with_db, delete_user):
    decision_tree_id = delete_user["current"].id
    with auth_as(app_with_db, delete_user["user"]):
        r = await _delete_workflow(client, decision_tree_id, confirm_phrase="wrong phrase")
...
async def test_delete_non_author_returns_403(client, app_with_db, delete_user, test_session_factory):
    ...
    with auth_as(app_with_db, other):
        r = await _delete_workflow(client, decision_tree_id, confirm_phrase=DELETE_CONFIRM_PHRASE)
...
async def test_delete_non_current_version_returns_400(client, app_with_db, delete_user):
    decision_tree_id = delete_user["root"].id
    with auth_as(app_with_db, delete_user["user"]):
        r = await _delete_workflow(client, decision_tree_id, confirm_phrase=DELETE_CONFIRM_PHRASE)
    assert r.status_code == 400
...
async def test_delete_removes_entire_family_and_related_rows(
    ...
    async with test_session_factory() as session:
        ...
        remaining_artifacts = await session.execute(
            select(func.count(ReasoningCompiledArtifact.decision_tree_id)).where(
                ReasoningCompiledArtifact.decision_tree_id.in_(family_ids)
            )
        )
