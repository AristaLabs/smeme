#!/usr/bin/env python3
"""Load a saved QNRGraph JSON, compile to IR, print IR + validation + enumeration + witness.

Usage:
  uv run python scripts/qnr_reasoning_inspect.py
  uv run python scripts/qnr_reasoning_inspect.py /path/to/qnr_graph.json

Default JSON: tests/fixtures/reasoning/test_qnr_graphs/sample_qnr_graph.json (two-branch radio).
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Print QNR → IR and Phase 1 reasoning outputs.")
    parser.add_argument(
        "json_path",
        nargs="?",
        type=Path,
        default=None,
        help="Path to QNRGraph JSON (default: bundled sample fixture)",
    )
    ...
    graph = QNRGraph.model_validate_json(raw)

    print("=== QNRGraph (normalized dump) ===")
    print(json.dumps(graph.model_dump(mode="json"), indent=2))
    print()

    ir = compile_qnr_to_ir(graph)

    print("=== IR (compiled from QNR) ===")
